...
public class TokenSequence extends ArrayList<Token> implements Sequence, Serializable {
	...
	public TokenSequence (String[] tokens) {
		this( tokens.length );
		for (int i = 0; i < tokens.length; i++)
			this.add (new Token( tokens[i]));
	}	
	...
	*/
	...
	public void add(String string, double weight, int chunkID, int lineID) {
		add(new Token(string, weight, chunkID, lineID));
	}
