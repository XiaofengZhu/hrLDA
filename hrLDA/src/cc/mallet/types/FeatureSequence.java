...
public class FeatureSequence implements Sequence, Serializable, AlphabetCarrying
{
	 ...
	 */
	public FeatureSequence (Alphabet dict, int[] features, double [] weights, int [] chunkIDs, int[] sentenceIDs)
	{
		this(dict, features.length);
		for (int i = 0; i < features.length; i++){
//			add(features[i]);
			add(features[i], weights[i], chunkIDs[i],sentenceIDs[i] );
//			this.weights [i] = weights[i];
//			this.chunkIDs [i] = chunkIDs[i];
//			this.sentenceIDs [i] = sentenceIDs[i];
			}
	}
	...
	public FeatureSequence (Alphabet dict, int capacity)
	{
		...
		weights = new double[capacity > 2 ? capacity : 2];
		chunkIDs = new int[capacity > 2 ? capacity : 2];
		sentenceIDs = new int[capacity > 2 ? capacity : 2];
		length = 0;
	}
	...
	public final double getWeightAtPosition (int pos)
	{
		return weights[pos];
	}
	public final int getChunkIDAtPosition (int pos)
	{
		return chunkIDs[pos];
	}
	public final int getSentenceIDAtPosition (int pos)
	{
		return sentenceIDs[pos];
	}	
	...
	public void add (int featureIndex, double weight, int chunkID, int sentenceID){
		growIfNecessary ();
		assert (featureIndex < dictionary.size());
		length++;
		features[length] = featureIndex;	
		weights[length] = weight;
		chunkIDs[length] = chunkID;
		sentenceIDs[length] = sentenceID;
	}
	...
	public void add (String key, double weight, int chunkID, int sentenceID){
//		System.out.println("THIS IS A TEST!!!!! ");
//		System.out.println("key: "+key+ " weight: "+weight+ " chunkID: "+chunkID+ " sentenceID: "+sentenceID);
		int fi = dictionary.lookupIndex (key);
		if (fi >= 0){
			growIfNecessary ();
			assert (fi < dictionary.size());
