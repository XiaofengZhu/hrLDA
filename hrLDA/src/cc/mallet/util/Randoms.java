...
//	}
  ...
  public NCRPNode nextDiscrete(HashMap<NCRPNode, Double> leafWeights, double sum) {
		double b = 0, r = nextUniform() * sum;
		NCRPNode result = null;
		for (NCRPNode ncrp : leafWeights.keySet()) {
			b += leafWeights.get(ncrp);
			if (b > r) {
			    return ncrp;
			}
			result = ncrp;
